...
async def split_file(file_path, chunk_size_mb=2000):
    ...
    if file_size > chunk_size:
        ...
        # Use ffmpeg to split the file to maintain video integrity
...
async def download_video(url, cmd, name):
    ...
    while not success and retry_count < max_retries:
        try:
            if "media-cdn.classplusapp.com" in url or "media-cdn-alisg.classplusapp.com" in url:
            ...
            print(f"\n⚡ Download attempt {retry_count + 1}")
            ...
            if not success:
                print(f"\n⚠️ Attempt {retry_count + 1} failed, retrying...")
                retry_count += 1
                await asyncio.sleep(3)
...
async def send_vid(bot: Client, m: Message, cc, filename, thumb, name, prog, channel_id):
    ...
    if isinstance(filename, list) and len(filename) > 1:
        ...
        await m.reply_text(f"**Large File Detected!**\nFile will be uploaded in {len(filename)} parts...")
        
        for i, part in enumerate(filename, 1):
            # Generate thumbnail for first part only
            subprocess.run(f'ffmpeg -i "{part}" -ss 00:00:10 -vframes 1 "{part}.jpg"', shell=True)
            if thumb == "/d" and os.path.exists("wm.png"):
                watermark_cmd = [
        'ffmpeg',
        '-i', f'{part}_temp.jpg',
        '-i', 'wm.png',
        '-filter_complex',
        '[1]scale=1400:160[wm];[0][wm]overlay=0:0',
        '-frames:v', '1',
        '-update', '1',
        '-y',
        f'{part}_watermarked.jpg'
    ]
                subprocess.run(watermark_cmd)
                thumbnail = f"{part}_watermarked.jpg"
            else:
                thumbnail = thumb if os.path.exists(thumb) else f"{part}_temp.jpg"
            ...
            if i == 1 and os.path.exists(f"{part}.jpg"):
                os.remove(f"{part}.jpg")
            
            # Small delay between parts
            await asyncio.sleep(3)
        
        reply = await m.reply_text("**✅ All parts uploaded successfully!**")
        await asyncio.sleep(2)
        await reply.delete(True)

    else:
        # Original code for single file
        if isinstance(filename, list):
            filename = filename[0]
            
        # Generate thumbnail with watermark if needed
        try:
            # First extract thumbnail frame
            subprocess.run(f'ffmpeg -i "{filename}" -ss 00:00:10 -vframes 1 -s 1280x720 "{filename}_temp.jpg" -y', shell=True)
            
            # Handle watermark based on thumb parameter
            if thumb == "/d" and os.path.exists("wm.png"):
                # Use default watermark
                watermark_cmd = [
    'ffmpeg',
    '-i', f'{filename}_temp.jpg',   # Input thumbnail
    '-i', 'wm.png',                 # Input watermark
    '-filter_complex',
    '[1]scale=1400:160[wm];[0][wm]overlay=0:0',  # Zoom watermark + place at top
    '-frames:v', '1',               # Output one frame
    '-update', '1',                 # Enable update mode
    '-y',                           # Overwrite output
    f'{filename}_watermarked.jpg'   # Output
]
                ...
                if thumb == "no" or thumb == "No" and os.path.exists(thumb):
                    watermark_cmd = [
                        'ffmpeg',
                        '-i', f'{filename}_temp.jpg',  # Input thumbnail
                        '-i', thumb,  # Input custom watermark
                        '-filter_complex',
                        '[1]scale=1280:150[wm];[0][wm]overlay=0:40',  # Scale watermark and position it
                        '-frames:v', '1',
                        '-update', '1',
                        '-y',
                        f'{filename}_watermarked.jpg'
                    ]
                    subprocess.run(watermark_cmd)
                    thumbnail = f"{filename}_watermarked.jpg"
                else:
                    # No watermark
                    thumbnail = f"{filename}_temp.jpg"
                
            # Clean up temp file
            if os.path.exists(f"{filename}_temp.jpg") and thumbnail != f"{filename}_temp.jpg":
                os.remove(f"{filename}_temp.jpg")
                
        except Exception as e:
            print(f"Error handling thumbnail: {str(e)}")
            # Fallback to simple thumbnail without watermark
        ...
        if os.path.exists(f"{filename}_watermarked.jpg"):
            os.remove(f"{filename}_watermarked.jpg")
