...
BUTTONSCONTACT = InlineKeyboardMarkup([[InlineKeyboardButton(text="📞 Contact", url="https://t.me/ITSUGBOT")]])
...
async def start(bot, m: Message):
    try:
        ...
        start_message = await m.reply_text("ɪɴɪᴛɪᴀʟɪᴢɪɴɢ ᴘʀᴇᴍɪᴜᴍ ᴀᴄᴄᴇss...")
        ...
        try:
            ...
            await m.reply_photo(
                ...
                caption=welcome_msg,
        reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("📥 ᴅᴏᴡɴʟᴏᴀᴅ ᴅʀᴍ", callback_data="start_drm")],
                    [
                        InlineKeyboardButton("📢 ᴜᴘᴅᴀᴛᴇs", url=PREMIUM_CHANNEL),
                        InlineKeyboardButton("📞 ᴄᴏɴᴛᴀᴄᴛ", url="https://t.me/ITSUGBOT")
                    ]
                ])
            )
        except Exception as e:
            ...
            await start_message.edit_text(
                "⚠️ ᴇʀʀᴏʀ sʜᴏᴡɪɴɢ ᴡᴇʟᴄᴏᴍᴇ ᴍᴇssᴀɢᴇ. ᴘʟᴇᴀsᴇ ᴛʀʏ ᴀɢᴀɪɴ ʟᴀᴛᴇʀ.",
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("🔄 ᴛʀʏ ᴀɢᴀɪɴ", callback_data="refresh_start")]
                ])
            )
    except Exception as e:
        print(f"Error in start command: {str(e)}")
        await m.reply_text("⚠️ ᴀɴ ᴇʀʀᴏʀ ᴏᴄᴄᴜʀʀᴇᴅ. ᴘʟᴇᴀsᴇ ᴛʀʏ ᴀɢᴀɪɴ ʟᴀᴛᴇʀ.")
...
async def start_drm(client, query):
    try:
        ...
        await query.message.reply_text("⚠️ ᴀɴ ᴇʀʀᴏʀ ᴏᴄᴄᴜʀʀᴇᴅ. ᴘʟᴇᴀsᴇ ᴛʀʏ ᴀɢᴀɪɴ.")
...
async def auto_default_handler(client, callback_query):
    global auto_clicked
    if callback_query.data == "auto_default":
        await callback_query.answer("Auto mode activated", show_alert=False)
        auto_flags[callback_query.message.chat.id] = True
...
async def txt_handler(bot: Client, m: Message):  
    ...
    reply_markup=InlineKeyboardMarkup(
        [[
            InlineKeyboardButton("👑 Join Premium Group", url="https://t.me/ITSUGBOT")
        ]]
    )
)
    ...
    reply_markup=InlineKeyboardMarkup([
        [InlineKeyboardButton("⚡ ᴀᴜᴛᴏ ᴅᴇꜰᴀᴜʟᴛ ᴀʟʟ", callback_data="auto_default")]
    ])
)
    ...
    try:
        input6: Message = await bot.listen(editable.chat.id, timeout=timeout_duration)
    ...
    try:
        watermark_msg: Message = await bot.listen(editable.chat.id, timeout=timeout_duration)
    ...
    try:
        for i in range(arg-1, len(links)):
            ...
            if "acecwply" in url:
                ...
                print("\n🔐 Detected DRM URL. Fetching keys via API...")
                ...
                while True:
                    try:
                        retry_count += 1
                        print(f"⏳ Attempt {retry_count}: Requesting DRM keys...")
                        mpd, keys = helper.get_mps_and_keys(api_url)

                        if mpd and keys:
                            url = mpd
                            keys_string = " ".join([f"--key {key}" for key in keys])
                            print("✅ Successfully fetched DRM keys and MPD URL!")
                            break  # Exit loop
        
                        else:
                            print("❌ DRM keys not found in response. Retrying in 5 seconds...")
                            await asyncio.sleep(5)
        
                    except Exception as e:
                        print(f"⚠️ Error while fetching DRM keys: {e}")
                        print("🔁 Retrying in 5 seconds...")
                        await asyncio.sleep(5)

            # Optional timeout check to avoid infinite hang
                        if time.time() - start_time > max_wait_time:
                            print("⏰ Timeout: Could not fetch DRM keys within 3 minutes.")
                            break
...
async def help_info(client, query):
    try:
        ...
        await query.message.edit_text(
            ...
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("💫 Get Premium Access", url="https://t.me/ITSUGBOT")],
                [InlineKeyboardButton("🔙 Back", callback_data="refresh_start")]
            ])
        )
